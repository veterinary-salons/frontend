...
import { setArticleTitle } from '../../app/store/articleTitle';
...
const Article = ({ article, withMargins }) => {
  ...
  // Установка названия статьи в Redux при монтировании компонента
  useEffect(() => {
    dispatch(setArticleTitle(article.title));

    // Функция очистки, которая будет вызвана при размонтировании компонента
    return () => {
      // Сбросить название статьи при размонтировании
  ...
  return (
    <section
      className={classNames(
        classes.article,
        withMargins && classes.article__margins,
      )}
    >
      <div className={classes.article__header}>
        ...
        <p className={classes['article__prop-text']}>
          Источник:&nbsp;
          <Link className={classes.article__link} to={source} target="_blank">
            {link}
          </Link>
        </p>
      </div>
      ...
      <div className={classes.article__like}>
        <Heart isActive={isLikedArticle} onClick={handleClickLike} />
